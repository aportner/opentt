...
Room.prototype.nextSong = function ()
{
    ...
    if ( this.currentDJ && this.currentSong )
    {
        this.currentDJ.isCurrentlyDJing = false;

        var playlist    = this.currentDJ.user.getActivePlaylist();
...
Room.prototype.nextSongDJDone = function()
{
    var _this           = this;
    ...
    var playlist        = this.currentPlaylist = this.currentDJ.user.getActivePlaylist();
    this.db.song.findOne({ _id: playlist.songs[ 0 ] }, function( err, song )
    {
        if ( !err && song )
        {
...
Room.prototype.getCurrentSong = function()
{
    ...
    obj.playlist        = this.currentPlaylist.name;
...
{
    var _this       = this;
    var playlist    = handler.user.getActivePlaylist();
    var id          = mongoose.Types.ObjectId( songId );
    var found       = false;

    playlist.songs.forEach( function( song )
    {
        if ( !found && song.equals( id ) )
        {
            found   = true;
            console.log( 'found', songId, song );
        };
    });
